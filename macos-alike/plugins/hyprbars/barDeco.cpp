...
void CHyprBar::onTouchMove(Event::SCallbackInfo& info, ITouch::SMotionEvent e) {
    ...
    if (!m_bDraggingThis) {
        ...
        Log::logger->log(Log::DEBUG, "Dragging window started");
...
void CHyprBar::handleUpEvent(Event::SCallbackInfo& info) {
    ...
    const bool BUTTONSRIGHT = ALIGNBUTTONS != "left";
    const bool IS_LEFT_BUTTON =

    if (doButtonRelease(BARPADDING, BARBUTTONPADDING, HEIGHT, COORDS, BUTTONSRIGHT, BUTTONSRIGHT))
        return;
...
bool CHyprBar::doButtonRelease(Config::INTEGER barPadding, Config::INTEGER barButtonPadding, Config::INTEGER barHeight, Vector2D COORDS, bool BUTTONSRIGHT, bool IS_LEFT_BUTTON){
    ...
    for (size_t i = 0; i < g_pGlobalState->buttons.size(); ++i) {
        ...
        if (VECINRECT(COORDS,
                      currentPos.x,
                      currentPos.y,
                      currentPos.x + b.size + barButtonPadding,
                      currentPos.y + b.size)) {
...
bool CHyprBar::doButtonPress(Config::INTEGER barPadding, Config::INTEGER barButtonPadding, Config::INTEGER barHeight, Vector2D COORDS, bool BUTTONSRIGHT) {
    float offset = barPadding;
    m_pressedButtonIndex = -1;
    ...
    for (size_t i = 0; i < g_pGlobalState->buttons.size(); ++i) {
        ...
        Vector2D currentPos = Vector2D{
            BUTTONSRIGHT ? BARBUF.x - barButtonPadding - b.size - offset : offset,
            (BARBUF.y - b.size) / 2.0
        }.floor();
...
void CHyprBar::renderBarButtons(CBox* barBox, const float scale, const float a) {
    ...
    m_bWindowHasFocus = (PWINDOW && Desktop::focusState()->window() == PWINDOW);
    ...
    for (size_t i = 0; i < visibleCount; ++i) {
        ...
        auto       bordercolor = button.bordercol;
        auto       bordersize = button.bordersize;
        ...
        CBox buttonBoxBorder = {barBox->x + (BUTTONSRIGHT ? barBox->w - offset - scaledButtonSize : offset), barBox->y + (barBox->h - scaledButtonSize) / 2.0, scaledButtonSize,
                          scaledButtonSize};

        buttonBox.round();
        buttonBoxBorder.round();

        if ((int)i == m_pressedButtonIndex) {
            color.r *= 0.7f;
            color.g *= 0.7f;
            color.b *= 0.7f;
            bordercolor = CHyprColor(0.3f, 0.3f, 0.3f, 1);
        }
        ...
        if (!m_bWindowHasFocus) {
            ...
            color.g = 0.3f;
            color.b = 0.3f;
            bordercolor = CHyprColor(0.2f, 0.2f, 0.2f, 0.8f);
        }

        // Draw button on top of the border
        g_pHyprOpenGL->renderRect(buttonBox, color, {.round = static_cast<int>(std::round(scaledButtonSize / 2.0)), .roundingPower = 2.0f});

        // Draw border
        g_pHyprOpenGL->renderBorder(buttonBoxBorder, bordercolor, {.round = static_cast<int>(std::round(scaledButtonSize / 2.0)), .roundingPower = 2.0f, .borderSize = bordersize});
