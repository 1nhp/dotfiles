...
Hyprlang::CParseResult onNewButton(const char* K, const char* V) {
    ...
    // hyprbars-button = bgcolor, size, icon, action, fgcolor, bordercolor, bordersize
    ...
    auto bordercolor = Config::ParserUtils::parseColor(vars[0]);
    ...
    if (!bordercolor) {
        result.setError("invalid bordercolor");
        return result;
    }

    if (!bordersize) {
        result.setError("invalid bordersize");
        return result;
    }

    g_pGlobalState->buttons.push_back(SHyprButton{vars[3], userfg, *fgcolor, *bgcolor, *bordercolor, bordersize, size, vars[2]});
...
int newLuaButton(lua_State* L) {
    if (!lua_istable(L, 1))
        return Config::Lua::Bindings::Internal::configError(L, "add_button: expected a table { bg_color, fg_color, border_color, size, icon, action }");
...

    {
        ...
        auto                         err = parser.parse(L);
        if (err.errorCode != Config::Lua::PARSE_ERROR_OK)
            return Config::Lua::Bindings::Internal::configError(L, "add_button: failed to parse border_color");

        button.bordercol = parser.parsed();
    }

    {
        Hyprutils::Utils::CScopeGuard x([L] { lua_pop(L, 1); });

        lua_getfield(L, 1, "border_size");

        if (!lua_isnumber(L, -1))
            return Config::Lua::Bindings::Internal::configError(L, "add_button: bordersize must be an integer");

        button.bordersize = lua_tointeger(L, -1);
    }
